...
| 25 | **CTA-Klicks werden zentral gezählt** (ein Klick-Listener auf Links zu `/einreichen` in `components/PlausibleCTATracking.tsx`), nicht Button für Button | es gibt 12 CTAs, und die geplanten Branchen- und Städte-Seiten bringen hunderte weitere; einzeln markierte Buttons würden dort regelmäßig vergessen. Zentral zählt jeder künftige CTA automatisch mit, ohne dass eine Seitendatei angefasst wird | falls einmal ein Link auf `/einreichen` zeigt, der *kein* CTA ist (dann würde er mitgezählt) |
| 26 | **Eigene Danke-Seite `/einreichen/danke`** statt eines Custom Events für den Abschluss; sie nutzt über `components/EinreichenContent.tsx` denselben Upload-Bereich wie `/einreichen` und nimmt sofort weitere Rechnungen entgegen | ein Seitenaufruf ist robuster zu messen als ein Ereignis; die gemeinsame Komponente verhindert, dass zwei Fassungen der Einreichungsseite auseinanderlaufen. Die Seite steht auf `noindex` mit `canonical` auf `/einreichen`, damit sie der echten Seite bei Google keine Konkurrenz macht | — |
