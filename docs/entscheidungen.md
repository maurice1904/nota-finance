...
| 21 | **Hochgeladene Datei wird nie als Reaktion auf einen internen Fehler gelöscht; Kunden-Erfolg hängt nur am Datei-Upload** *(Präzisierung August 2026, keine inhaltliche Änderung: gemeint war immer die Fehlerreaktion. Die planmäßige Löschung nach Fristablauf regelt Nr. 29.)* | die Datei ist der Fall — sie zu löschen (z. B. bei DB-Fehler) würde den Fall zerstören; ein interner Fehler darf den Kunden nicht mit einer Fehlermeldung bestrafen | — |
...
| 29 | **Automatische Löschung nach 90 Tagen, mit Aufbewahrungs-Haken je Einreichung** | Zweckfortfall nach Art. 17 DSGVO; die pauschale Frist allein wäre falsch (`recht-und-datenschutz.md` 2.5), der Haken nimmt laufende und titulierte Forderungen aus. Ist eine Datei markiert, bleibt die ganze Einreichung erhalten — Foto-PDF und Originalbilder gehören zusammen | wenn der Anwalt im Rahmen von P0-7 andere Fristen festlegt |
| 30 | **Löschen über die Storage-Schnittstelle, nicht per SQL; ausgelöst durch Vercel-Cron statt pg_cron** | ein `delete` auf `storage.objects` entfernt nur den Eintrag, die Datei bliebe unerreichbar liegen; pg_cron ist im kostenlosen Tarif zwar verfügbar, kann aber nur SQL ausführen und käme ohne `pg_net` nicht an die Schnittstelle | nie — technische Gegebenheit |
| 31 | **Präzisierung zu Entscheidung 3 (keine inhaltliche Änderung, August 2026):** „Marken bleiben getrennt" meint einen getrennten **Marktauftritt** — kein gemeinsames Angebot, keine Vermischung im Kundenerlebnis. Es bedeutet **nicht**, dass gerichtsmahnverfahren.de nicht als **Kompetenz- und Historien-Beleg** für twenty4collect genannt werden darf (Faktenkern, Autorenprofil, Presseberichte in `docs/SEO_Umsetzungskonzept.md`) | belegt Sachkunde und Historie des Geschäftsführers gegenüber Suchmaschinen und KI-Systemen, ohne die Marken im Kundenerlebnis zu vermischen — bloße Nennung als Beleg ist kein Marktauftritt | wenn daraus ein gemeinsamer Marketingkanal statt eines reinen Kompetenznachweises wird |
| 32 | **Kein Autorenprofil, keine Personennennung auf Fach-/Ratgeber-Seiten** — Teil 1.3 des SEO-Konzepts (Profilseite `/ueber/manfred-eberhard`, `Person`-Auszeichnung in JSON-LD, Autorenzeile auf Fachseiten) entfällt vollständig. Fach-/Ratgeber-Seiten tragen stattdessen nur ein Veröffentlichungs- und Aktualisierungsdatum, ohne Namensnennung | Wunsch des Auftraggebers (Grund im Auftrag nicht weiter spezifiziert) | wenn der Auftraggeber ein Autorenprofil ausdrücklich neu beauftragt |
